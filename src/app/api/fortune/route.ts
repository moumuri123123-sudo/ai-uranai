...
function buildTarotPrompt(
  ...
  if (tarotSpread === 'three' && tarotCardsArr && tarotCardsArr.length === 3) {
    const cardDescriptions = tarotCardsArr
      .map((c) => {
    ...
    const conversationContext =
      messages.length > 0
        ? '\n\n【これまでの会話】\n' +
          messages
            .map(
              (m) => `${m.role === 'user' ? '相談者' : '占い師'}: ${sanitizeUserInput(m.content)}`,
            )
            .join('\n')
        : '';
...
カードの解釈をさらに深掘りしたり、別の観点からアドバイスを加えてください。`
  ...
  const conversationContext =
    messages.length > 0
      ? '\n\n【これまでの会話】\n' +
        messages
          .map((m) => `${m.role === 'user' ? '相談者' : '占い師'}: ${sanitizeUserInput(m.content)}`)
          .join('\n')
      : '';
...
export async function POST(req: Request) {
  try {
    ...
    if (!body.type || !isValidFortuneType(body.type)) {
      return Response.json({ error: '占いの種類を指定してください。' }, { status: 400 });
    }
    ...
    if (body.question.length > 500) {
      return Response.json({ error: '質問は500文字以内で入力してください。' }, { status: 400 });
    }

    // 数秘術の場合は生年月日の形式と値を厳密にチェック
    if (body.type === 'numerology') {
      if (typeof body.birthDate !== 'string' || !isValidBirthDate(body.birthDate)) {
        return Response.json({ error: '生年月日の形式が正しくありません' }, { status: 400 });
      }
    }
    ...
    switch (request.type) {
      case 'tarot': {
        const prompt = buildTarotPrompt(
          ...
          request.tarotTheme,
          request.tarotCard,
          request.tarotReversed,
          request.tarotCards,
          request.tarotSpread,
          request.tarotQuestion,
        );
    ...
    const safeSystemInstruction =
      systemInstruction + turnGuideline + SAFETY_GUIDELINE + INJECTION_GUARD;
