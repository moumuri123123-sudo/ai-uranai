...
export async function GET(request: Request) {
  ...
  const safeGrade = (v: string | null) => (v && VALID_GRADES.has(v) ? v : '○');
  ...
  const diffColor = diff > 0 ? '#ffd700' : diff < 0 ? '#8a7a6a' : '#c8a882';
  ...
  return new ImageResponse(
    ...
    >
      ...
      >
        ...
        >
          <div style={{ fontSize: 24, color: '#c8a882', display: 'flex' }}>仕事運</div>
          <div style={{ fontSize: 48, color: '#ffd700', display: 'flex' }}>{work}</div>
        </div>
        <div
          style={{
            display: 'flex',
        ...
        >
          <div style={{ fontSize: 24, color: '#c8a882', display: 'flex' }}>恋愛運</div>
          <div style={{ fontSize: 48, color: '#ff69b4', display: 'flex' }}>{love}</div>
        </div>
        <div
          style={{
            display: 'flex',
        ...
        >
          <div style={{ fontSize: 24, color: '#c8a882', display: 'flex' }}>金運</div>
          <div style={{ fontSize: 48, color: '#f0a030', display: 'flex' }}>{money}</div>
        </div>
      </div>
