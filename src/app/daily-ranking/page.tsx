...
async function getCachedComments(
  ...
  const cached = unstable_cache(
    async (d: string, r: { name: string; rank: number }[]) => generateAllCommentsRaw(d, r),
...
async function getCachedDetails(
  ...
  const cached = unstable_cache(
    async (d: string, r: ZodiacRanking[]): Promise<[string, ZodiacDetail][]> => {
      void d;
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-12 sm:py-16">
      ...
      <div className="mb-10 text-center">
        <p className="text-muted mb-2 text-sm tracking-widest">━━━ 毎朝7時更新 ━━━</p>
        <h1 className="font-mincho text-neon-red mb-2 text-3xl font-bold tracking-wider sm:text-4xl">
          今日の運勢ランキング
        </h1>
        <p className="font-yuji text-gold text-xl">
          {month}月{day}日
        </p>
        <p className="text-muted mt-2 text-xs">星座をタップすると詳細運勢が開きます</p>
      </div>
      ...
      <div className="mt-8 text-center">
        <Link
          href="/zodiac"
          className="border-neon-red/50 text-neon-red hover:bg-neon-red/10 inline-block rounded-full border-2 px-8 py-3 text-base font-bold transition-colors"
        >
          自分の星座について詳しく占う
        </Link>
      </div>
      ...
      <div className="border-border bg-surface mt-8 rounded-xl border p-6 text-center">
        <p className="text-muted mb-4 text-sm">もっと詳しく占いたい方はこちら</p>
        <div className="flex flex-wrap justify-center gap-3">
          <Link
            href="/tarot"
            className="border-neon-red/30 text-neon-red hover:bg-neon-red/10 rounded-full border px-4 py-2 text-sm transition-colors"
          >
            タロット占い
          </Link>
          <Link
            href="/numerology"
            className="border-neon-purple/30 text-neon-purple hover:bg-neon-purple/10 rounded-full border px-4 py-2 text-sm transition-colors"
          >
            数秘術
          </Link>
        </div>
