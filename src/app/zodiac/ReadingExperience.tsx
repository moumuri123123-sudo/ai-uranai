...
const ChatBox = dynamic(() => import('@/components/ChatBox'), {
  ...
  loading: () => (
    <div
      className="border-gold/30 bg-surface mx-auto my-6 h-24 w-full max-w-2xl animate-pulse rounded-2xl border"
    ...
    <>
      ...
      {!selectedSign && (
        <div className="mx-auto max-w-2xl">
          <h2 className="text-warm mb-6 text-center text-sm font-medium">
            あなたの星座を選んでください
          </h2>
          <div className="grid grid-cols-3 gap-3 sm:grid-cols-4">
            {zodiacList.map((sign) => (
              <button
                ...
                className="group border-border bg-surface hover:border-gold/50 hover:bg-surface-hover hover:shadow-gold/10 flex min-h-11 flex-col items-center gap-2 rounded-xl border p-4 transition-all hover:shadow-lg active:scale-95"
              >
                <span
                  aria-hidden="true"
                ...
                <span className="text-foreground/90 text-sm font-medium">{sign.name}</span>
                <span className="text-muted text-[10px]">{sign.period}</span>
              </button>
            ))}
          </div>
        </div>
      ...
      {selectedSign && selected && (
        <div>
          <div className="border-gold/30 bg-surface shadow-gold/10 mx-auto mb-8 max-w-sm rounded-2xl border p-6 text-center shadow-lg">
            <span className="text-4xl" aria-hidden="true">
              {zodiacEmojis[selectedSign]}
            </span>
            <p className="text-gold mt-2 text-xl font-bold">{selected.name}</p>
