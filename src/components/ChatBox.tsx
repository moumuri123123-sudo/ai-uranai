...
export default function ChatBox({
  ...
  const samples =
    sampleQuestions && sampleQuestions.length > 0 ? sampleQuestions : DEFAULT_SAMPLE_QUESTIONS;

  return (
    <div className="border-border mx-auto w-full max-w-2xl overflow-hidden rounded-2xl border bg-[#0a0408] shadow-2xl">
      {/* メッセージ一覧 + sticky入力（同じスクロールコンテキストに入れることでstickyが機能する） */}
      <div
        ref={messagesContainerRef}
        className="scrollbar-thin relative max-h-[70vh] overflow-y-auto sm:max-h-[600px]"
      >
        <div className="space-y-1 p-4" role="log" aria-live="polite" aria-relevant="additions">
          {messages.length === 0 && (
            <div className="text-muted flex min-h-[14rem] flex-col items-center justify-center py-8">
              <span
                className="text-gold mb-3 text-4xl motion-safe:animate-pulse"
                aria-hidden="true"
              >
                &#x2726;
              </span>
              <p className="font-mincho text-warm text-sm">ようこそ、占処へ。</p>
              <p className="text-muted mt-1 text-xs">気になることを自由に尋ねてみてください</p>

              {/* サンプル質問チップ */}
              <div className="mt-5 flex w-full max-w-md flex-col gap-2">
                <p
                  className="text-gold/80 text-center text-[10px] tracking-widest"
                  aria-hidden="true"
                >
                ...
                <div className="flex flex-wrap justify-center gap-2">
                  {samples.map((q) => (
                    <button
                      ...
                      className="border-border bg-surface text-warm hover:border-neon-red/50 hover:bg-neon-red/5 hover:text-neon-red min-h-10 rounded-full border px-3.5 py-1.5 text-xs transition-all active:scale-95"
                    >
                      {q}
                    </button>
                  ))}
          ...
          {isWaitingForStream && (
            <div
              className="text-muted mt-2 flex items-center gap-2 px-2 text-xs"
              aria-live="polite"
              role="status"
            >
              <span
                aria-hidden="true"
                className="bg-neon-red inline-block h-2 w-2 rounded-full motion-safe:animate-pulse"
              />
          ...
          {showAfterContent && <div className="mt-6">{afterContent}</div>}
        </div>

        {/* 入力エリア（メッセージと同じスクロール親内にstickyで常時表示） */}
        <form
          onSubmit={handleSubmit}
          className="border-border sticky bottom-0 z-10 flex items-end gap-2 border-t bg-[#0a0408]/85 p-3 backdrop-blur-md supports-[backdrop-filter]:bg-[#0a0408]/70"
        >
          ...
          <textarea
            ...
            rows={1}
            aria-label="質問を入力"
            className="bg-surface border-border text-foreground placeholder-muted/60 focus:border-neon-red/50 focus:ring-neon-red/30 flex-1 resize-none rounded-xl border px-4 py-2.5 text-sm transition-colors focus:ring-1 focus:outline-none disabled:opacity-50"
          />
          {input.length > 400 && (
            <span
              className={`absolute -top-5 right-3 text-xs ${input.length >= 500 ? 'text-neon-red' : 'text-muted'}`}
            >
              {input.length}/500
            </span>
          )}
          <button
            ...
            className="bg-neon-red flex h-11 w-11 flex-shrink-0 items-center justify-center rounded-xl text-white transition-all hover:brightness-110 active:scale-95 disabled:cursor-not-allowed disabled:opacity-40"
            aria-label="送信"
          >
            <svg
      ...
      {messages.some(
        (m) =>
          m.role === 'assistant' && m.content.trim() && !m.content.includes(STREAM_ERROR_MARKER),
      ) &&
