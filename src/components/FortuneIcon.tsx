...
export default function FortuneIcon({ type, size = 'lg', className = '' }: FortuneIconProps) {
  ...
  return (
    <div
      ...
      className={`border-border bg-surface mx-auto flex items-center justify-center rounded-full border shadow-lg ${icon.glow} ${sizes[size]} ${className}`}
    >
      <span className={`font-yuji ${icon.color}`} aria-hidden="true">
        {icon.char}
      </span>
    </div>
  );
