...
export default function NextFortuneCTA({ currentFortune }: NextFortuneCTAProps) {
  ...
  return (
    <section
      ...
      className="border-gold/30 bg-surface/60 shadow-gold/5 my-8 rounded-2xl border p-5 shadow-lg sm:p-6"
    >
      <div className="mb-5 text-center">
        <p className="text-gold/80 mb-1 text-[10px] tracking-widest" aria-hidden="true">
          &mdash; NEXT &mdash;
        </p>
        <h3 className="font-mincho text-warm text-base font-semibold sm:text-lg">
          もう少し、占ってみませんか？
        </h3>
      </div>
        ...
        {others.map((type) => {
          ...
          return (
            <li key={type}>
              <Link
                href={info.href}
                className="group border-border hover:border-neon-red/50 hover:shadow-neon-red/10 flex h-full min-h-[5.5rem] items-start gap-3 rounded-xl border bg-[#0a0408] p-4 transition-all hover:shadow-lg active:scale-[0.98]"
              >
                <span aria-hidden="true" className="text-gold flex-shrink-0 text-2xl leading-none">
                  {info.icon}
                </span>
                <span className="flex flex-col">
                  <span className="font-mincho text-foreground group-hover:text-neon-red text-sm font-bold transition-colors">
                    {info.label}
                  </span>
                  <span className="text-muted mt-1 text-xs leading-relaxed">{info.pitch}</span>
                </span>
              </Link>
            </li>
          );
      ...
      {articles.length > 0 && (
        <div className="border-border mt-5 border-t pt-4">
          <p className="text-gold mb-2 text-xs font-medium">関連コラム</p>
          <ul className="space-y-1.5">
            {articles.map((a) => (
              <li key={a.slug}>
                <Link
                  href={`/blog/${a.slug}`}
                  className="text-warm hover:text-neon-red inline-flex items-center gap-1.5 text-xs transition-colors"
                >
                  <span aria-hidden="true" className="text-gold/70">
                    &rsaquo;
                  </span>
                  <span className="decoration-border underline decoration-dotted underline-offset-2">
                    {a.title}
                  </span>
                </Link>
              </li>
