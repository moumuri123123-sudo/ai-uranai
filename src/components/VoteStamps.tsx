...
function getOrCreateVoterId(): string {
  ...
  try {
    ...
    if (!id) {
      ...
      id = Array.from(arr, (b) => b.toString(36))
        .join('')
        .slice(0, 32);
...
export default function VoteStamps({ fortuneType }: VoteStampsProps) {
  ...
  return (
    <section
      aria-label="この鑑定への共感度スタンプ"
      className="border-border bg-surface/40 mt-4 rounded-lg border p-4"
    >
      <p className="text-muted mb-3 text-center text-xs">この鑑定、どう感じましたか？</p>
          ...
          return (
            <button
              ...
              className={`flex min-h-11 min-w-[6rem] flex-col items-center justify-center gap-0.5 rounded-lg border px-3 py-2 transition-all ${
                isVoted
                  ? 'border-gold/60 bg-gold/10 cursor-default'
